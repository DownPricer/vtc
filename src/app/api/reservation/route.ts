...
export async function POST(request: Request) {
  try {
    ...
    if (process.env.NODE_ENV !== "production") {
      console.info("[next-api][reservation] pricingConfig injecte cote serveur");
    }
    ...
    if (isPricingConfigValidationFailure(res.status, json)) {
      ...
      console.warn("[next-api][reservation] pricingConfig invalide, fallback sans pricingConfig", {
        status: res.status,
        apiResponse: json,
      });
      const fallbackRes = await postCentralApiServer("reservation", clientPayload);
