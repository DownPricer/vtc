...
import { fetchPublicPaymentConfirmation, type PaymentConfirmationData } from "@/lib/publicPaymentConfirmation";
      ...
      <div className="max-w-lg w-full text-center">
        {loading && !data ? (
          ...
          <>
            ...
            {data.confirmationPending ? (
              <>
                ...
                <p className="text-gray-500 text-sm leading-relaxed mb-6">
                  Votre banque peut prendre quelques instants. La confirmation définitive arrive par e-mail dès que notre système a traité le paiement
                  (webhook Stripe).
                </p>
              </>
            ) : (
              <>
                ...
                <p className="text-gray-500 text-sm leading-relaxed mb-6">
                  Montant réglé : <span className="text-white font-semibold">{formatMoney(data.amount, data.currency)}</span>
                  <br />
            ...
            <Link href="/" className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl border border-white/10 text-white font-semibold text-sm hover:bg-white/5 transition-all">
              Retour à l&apos;accueil
            </Link>
          </>
        ) : null}
