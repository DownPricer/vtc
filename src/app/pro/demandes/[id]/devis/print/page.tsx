...
export default function ProDevisPrintOnlyPage() {
  ...
  const tarifTtc =
    item?.pricingResult && isUsefulValue((item.pricingResult as Record<string, unknown>).tarif)
  ...
  return (
    <ProGuard>
      {error ? <p className="mx-auto max-w-[210mm] px-6 py-10 text-sm text-rose-700">{error}</p> : null}

      {item ? (
        <div
          className="quote-print-area mx-auto max-w-[210mm] overflow-hidden bg-white px-8 py-10 text-slate-900 shadow-lg print:mx-0 print:max-w-none print:bg-white print:p-0 print:shadow-none"
          style={{ fontFamily: "Georgia, 'Times New Roman', serif" }}
        >
          <style
            dangerouslySetInnerHTML={{
              __html: `@page{size:A4;margin:12mm}@media print{html,body{background:#fff!important;color:#0f172a!important}}`,
            }}
          />
          ...
          <section className="print-card mt-8 grid gap-6 border-b border-slate-200 pb-6 md:grid-cols-2">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">{formatAddress(tenant.contact.address)}</p>
              {tenant.contact.emailPublic ? <p className="mt-1 text-sm text-slate-600">{tenant.contact.emailPublic}</p> : null}
            </div>
            <div>
          ...
          <section className="print-card mt-8 border-t border-slate-200 pt-6">
            ...
            {item.paymentStatus === "PAID" && paid ? (
              <div className="mt-6 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-950">
                ...
                {paidReceiptUrl ? (
                  <div className="mt-2">
                    <p className="font-semibold text-emerald-800">Reçu Stripe</p>
                    <p className="mt-1 text-[11px] text-emerald-900/80">{truncateReceiptUrl(paidReceiptUrl)}</p>
                  </div>
                ) : null}
              </div>
          ...
          <section className="print-card mt-10 border-t border-slate-200 pt-6 text-sm text-slate-600">
            ...
            <ul className="mt-3 list-disc space-y-2 pl-5">
              ...
              <li>Après acceptation, les conditions annulation / report suivent votre politique commerciale habituelle.</li>
        ...
        <p className="mx-auto max-w-[210mm] px-6 py-10 text-sm text-slate-600">Chargement du devis…</p>
      ) : null}
    </ProGuard>
  );
}
